function [T, theta_d, phi_d, p_errors] = position_controller(pos_err, state, des_pos, dt, params)
    %% Input Arguments
    ...
    %% Vertical Controller
        ...
        %T = uint8(max((K_p*z_curr_error) + (K_i*z_cumm_error*dt) + (K_d*(z_curr_error-z_prev_error)/dt), Min_THR));
    ...
    %% X-pos Controller
        % -------- Gainzzz --------
        K_p_x = 1.2*5.0;%5.0; %10.0;  %0.1 OC       %0.05
        K_i_x = 1.2*0.3;%0.3; %0.1;  %0.1 OC       %0.5
        K_d_x = 1.2*2.0;%2.0; %4.0;  %0.15 OC      %40.0
