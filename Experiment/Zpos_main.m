...
b = ble("C02874325133"); % Make sure to get the the Drone's MAC address before running this code
...
Drone_data = zeros(ITERATIONS + 2, 4);  % drone position data
...
%     v_y_d(k) = 0.01*0.5*cos(0.01*k_bar);
    ...
    % pass the data through a low pas filter
    [x_f, lpfData_x] = lpf_2(lpfData_x, DronePos(2));
    p_x(k) = x_f;
